...
if(isset($_POST['submit'])){
   ...
   if(!empty($profession)){
      $update_profession = $conn->prepare("UPDATE `admins` SET profession = ? WHERE id = ?");
   ...
   if($old_pass != $empty_pass){
      if($old_pass != $prev_pass){
         ...
         if($new_pass != $empty_pass){
            $update_pass = $conn->prepare("UPDATE `admins` SET password = ?, vpass = ? WHERE id = ?");
            $update_pass->execute([$cpass, $vpass, $admin_id]);
