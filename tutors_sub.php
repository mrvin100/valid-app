...
<head>
   ...
   <title>teachers subscripted</title>
   ...
   <div class="box-container">
      ...
      <?php
	    // for tutor subscripted
         $select_tutors = $conn->prepare(/*SELECT * FROM `subscribe` INNER JOIN `tutors` ON `subscribe`.tutor_id = `tutors`.id GROUP BY `tutors`.id ORDER BY COUNT(*) DESC LIMIT 9*/"
		 SELECT * FROM `subscribe` INNER JOIN `tutors` ON `subscribe`.tutor_id = `tutors`.id WHERE `subscribe`.user_id = ? GROUP BY `tutors`.id ORDER BY COUNT(*) DESC LIMIT 9;");
	   /*SELECT `subscribe`.user_id, `tutors`.name, COUNT(*) FROM `subscribe` INNER JOIN `tutors` ON `subscribe`.tutor_id = `tutors`.id GROUP BY `tutors`.id ORDER BY COUNT(*) DESC LIMIT 9;*/
         $select_tutors->execute([$user_id]);
      ...
      <?php
            ...
            echo '<p class="empty">vous n\'ete abonné(e) a aucun tuteur!</p>';
         }
      ?>
