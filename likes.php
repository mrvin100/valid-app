...
<section class="liked-videos">
   ...
   <div class="box">
      ...
      <a href="watch_video.php?get_id=<?= $fetch_contents['id']; ?>" title="lire <?= $fetch_contents['title']; ?>">
      <img src="uploaded_files/<?= $fetch_contents['thumb']; ?>" alt="" class="thumb">
      <h3 class="title"><?= $fetch_contents['title']; ?></h3></a>
      <form action="" method="post" class="flex-btn">
         <input type="hidden" name="content_id" value="<?= $fetch_contents['id']; ?>">
         <?php /*?><input type="submit" value="remove" class="inline-delete-btn" name="remove"><?php */?>
      </form>
   </div>
   <?php
            }
         }else{
            echo '<p class="emtpy">aucun contenu trouvé!</p>';         
         }
      }
   }else{
      echo '<p class="empty">pas encore d\'ajout aux likes!</p>';
   }
   ?>
